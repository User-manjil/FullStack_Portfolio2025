...
const TitleSection = (props) => {
  ...
  return (
    <div className='w-full mt-10 flex justify-between items-end'>
        ...
        <div className="flex w-10  h-[1px] bg-(--lime)  font-[poppins] "></div> {props.subTitle}
            </div>
        <div className="flex font-[Syne] text-(--text) font-black  md:text-6xl  ">{props.bigTitle}</div>
      ...
      <div className="flex  ">
        <Link to={props.link}>
           <button  className="font-medium w-fit border gap-2 group border-(--border2) flex py-2 px-5 h-10 text-(--muted2) hover:text-(--text)">
             {props.buttonContent}<p className="duration-300 group-hover:translate-x-0.5">→</p>
            </button>
        </Link>
